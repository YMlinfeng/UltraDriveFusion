...
class HunyuanVideoUpsampleCausal3D(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        if num_frames > 1:
            ...
            other_frames = other_frames.contiguous() # [1, 512, 2, 106, 200]
            other_frames = F.interpolate(other_frames, scale_factor=self.upsample_factor, mode="nearest") # [1, 512, 2, 212, 400],torch.Size([1, 256, 4, 424, 800])
...
class AutoencoderKLHunyuanVideo(ModelMixin, ConfigMixin):
    ...
    def __init__(
        ...
        self.use_slicing = True # 当同时解码一批视频 latent 时，可以通过在 batch 维度上进行分片，只对单个视频 latent 进行解码，从而节省内存
...
class VideoAutoencoderKLHunyuan(nn.Module):
    def __init__(
        self,
        ...
        super().__init__()
        self.module: AutoencoderKLHunyuanVideo = AutoencoderKLHunyuanVideo.from_pretrained(
            from_pretrained,
            cache_dir=cache_dir,
